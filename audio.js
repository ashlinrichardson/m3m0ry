...
var FastBase64 = {
  ...
  Encode: function(src) {
    var len = src.length; var dst = ''; var i = 0;
    while (len > 2) {
    ...
    if (len > 0) {
      var n1 = (src[i] & 0xFC) >> 2; var n2 = (src[i] & 0x03) << 4;
      if (len > 1) n2 |= (src[++i] & 0xF0) >> 4;
      dst += this.chars[n1]; dst += this.chars[n2];
      if (len == 2) {
        var n3 = (src[i++] & 0x0F) << 2;
        n3 |= (src[i] & 0xC0) >> 6;
        dst += this.chars[n3];
...
var RIFFWAVE = function(data) {
  ...
  function split16bitArray(data) {
    var r = []; var j = 0; var len = data.length;
    for (var i = 0; i < len; i++) {
      r[j++] = data[i] & 0xFF; r[j++] = (data[i] >> 8) & 0xFF;
    }
    return r;
  }
  this.Make = function(data) {
    ...
    this.wav = this.header.chunkId.concat(
      ...
      this.header.subChunk2Id, u32ToArray(this.header.subChunk2Size),
...
function createAudio() {
  function genAudio(frequency, scaleFactor, numberOfSamples) {
    ...
    var samples = generateSamples(sampleRate, numberOfSamples, frequency, scaleFactor);
    normalize_invalid_values(samples); // keep samples between [-1, +1]
    var wave = new RIFFWAVE(); wave.header.sampleRate = sampleRate;
    wave.header.numChannels = 1; var audio = new Audio();
    var samples2 = convert255(samples); wave.Make(samples2); 
...
/* play a soft click noise e..g, to indicate timer up */
function click(){ audio[1].play(); }
